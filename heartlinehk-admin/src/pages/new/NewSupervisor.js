...
const Supervisor = () => {
    ...
    const currentClient = useMemo(() => 
        (currentVolun && onlineVoluns.length > 0 && onlineVoluns.filter(value => value['volunId'] === currentVolun).length > 0 ? onlineVoluns.filter(value => value['volunId'] === currentVolun)[0]['clientId'] : null)
    , [onlineVoluns, currentVolun]);
    const currentMode = useMemo(() => 
        (currentVolun && onlineVoluns.length > 0 && onlineVoluns.filter(value => value['volunId'] === currentVolun).length > 0 ? onlineVoluns.filter(value => value['volunId'] === currentVolun)[0]['type'] : null)
    , [onlineVoluns, currentVolun]);
